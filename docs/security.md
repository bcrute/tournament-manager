...
| Provider choice | **Brevo** is the recommended default: EU-hosted (France), GDPR-native, 300/day free. Mailjet is the EU alternative; Resend, Postmark, SendGrid and SMTP2GO are listed and US-hosted. | Every message this app sends is an account-recovery message, so the provider sees an address and the fact that someone is recovering an account. Keeping that inside the EU means it answers to EU data protection law directly rather than through an adequacy decision that can be withdrawn. The registry exists so this choice is one word to revisit. |
