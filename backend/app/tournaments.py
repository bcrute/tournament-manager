...
def public_standings(code: str, entrants=None):
    ...
    `entrants` is passed straight through, so the snapshot below can hand over
    the roster it already holds instead of making the server read it twice.
    """
    return [
        {**{k: v for k, v in row.items() if k != "publicId"}, "entrantId": row["publicId"]}
        for row in standings_rows(code, entrants)
    ]
...
def pod_rows_for(round_row):
    """The pod and seat rows of one round: `(pods, seats_by_pod)`.

    Split out from the view below so a caller that has to build the same round
    for many viewers — the socket fanout — reads it once and personalizes each
    copy in memory.
    """
    pods, seats = [], []
    if round_row:
...
def pod_views_from(pods, by_pod, round_ends_at, viewer_entrant=None, organizer: bool = False):
    """One viewer's `(pods, myPod)` from rows already fetched. No database access.
...
def pod_views_for(round_row, viewer_entrant=None, organizer: bool = False):
    ...
    Reads the rows and applies the view rules in one call, for a caller serving
    exactly one viewer — the historical round view. The live snapshot splits the
    two halves so it can read once and personalize many times.
    """
    pods, by_pod = pod_rows_for(round_row)
    return pod_views_from(
        pods,
        by_pod,
        round_row["ends_at"] if round_row else None,
        viewer_entrant,
        organizer,
    )
...
def tournament_snapshot(code: str, with_calls: bool = False):
    ...
    pods, by_pod = pod_rows_for(latest)
    calls = (
        q(
    ...
    return {
        ...
        "by_pod": by_pod,
        "calls": calls,
        "standings": public_standings(t["code"], entrants),
        # the bracket is derived from pod results, so it is read here with
        # everything else rather than once per viewer
...
def personalize_tournament(snap, viewer_entrant=None, organizer: bool = False):
    ...
    Every §1 view rule lives here and in `pod_views_from` and nowhere else:
    `roomCode` is the organizer's, a room token appears only on the viewer's
    own seat, and the calls queue is organizer-only. The poll and the socket
    both come through this function precisely so the two views cannot drift
    apart — a fanout with its own idea of who may see what is how a seat
    credential leaks.
    """
    t = snap["t"]
    cfg = snap["cfg"]
    rounds = snap["rounds"]
    latest = snap["latest"]
    pod_views, my_pod = pod_views_from(
        snap["pods"],
        snap["by_pod"],
        latest["ends_at"] if latest else None,
        viewer_entrant,
        organizer,
    )
...
async def open_round(code: str, body: RoundBody, request: Request):
    ...
    if cut_seeds(t["code"]):
        ...
        opened = _open_bracket_round(t, cfg)
        await broadcast_tournament(t["code"])
        return opened
...
async def make_cut(code: str, body: CutBody, request: Request):
    ...
    # a bracket being drawn is the single biggest change a client can be shown;
    # nobody should learn they made the cut on the next poll
    await broadcast_tournament(t["code"])
    return {
        "ok": True,
        "cutTo": size,
